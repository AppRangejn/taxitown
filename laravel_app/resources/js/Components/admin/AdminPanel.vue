<template>
    <div>
        <!-- Floating Button для адміна -->
        <button
            v-if="!isAuthLoading && auth.user?.role === 'admin'"
            @click="isOpen = true"
            class="fixed bottom-5 right-5 z-50 w-16 h-16 bg-indigo-600 text-white rounded-full flex items-center justify-center shadow-2xl hover:bg-indigo-700 transition-transform transform hover:scale-110"
        >
